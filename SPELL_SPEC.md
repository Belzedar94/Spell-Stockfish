...
### 3.1c Freeze gate vs a live enemy zone
- The **exact center** of a live **enemy** freeze zone is **not a legal freeze gate**. Overlap is
  legal: only that one square is refused, so the counter-freeze can cover most of the same area
  from a neighbouring gate. The rule holds for zones clipped at an edge or corner too — the centre
  of a corner zone covering 4 squares is refused just the same.
- **Verified empirically on chess.com 2026-08-21** (analysis board, screenshots taken): after
  `1.freeze@e6 e4`, the client **rejects** a black `freeze@e6` and **accepts** `freeze@d6`, which
  shares 6 of e6's 9 squares. Black's freeze gates in that position are therefore **63**, not 64;
  one ply later, once the zone has expired, all 64 are back. Originally reported by sscg13.
- Only the enemy zone can ever apply. A caster's own zone expires at the end of the opponent's
  reply (§2.1), one ply before their own cooldown reaches 0, so `can_cast(us, FREEZE)` already
  implies the caster has no live zone. A symmetric self-guard would be dead code.
- Frozen as named cases in `tests/reference/perft_spell_freezegate.fens` and the fixed section of
  `tests/reference/perft_spell.csv`.
...
- a **gated move** `spell@gate,basemove`:
  ...
  - FREEZE gates: any board square (occupied allowed, `potionDropOnOccupied = true`) **except the
    centre of a live enemy freeze zone** (§3.1c). Filters on the casting ply: base `from != gate`,
    base `from` outside the new block zone, base `from` not already frozen (enemy zone), castling
    `to` not frozen.
...
```
depth 1: 1814   (= 20 normal + 1124 freeze-gated + 670 jump-gated)
depth 2: 3041742
```
Full root divide captured in `tests/reference/perft_divide_startpos_d1.txt` / `_d2.txt`.
Extended suite: `tests/reference/perft_spell.csv` (168 positions; the sampled sections come from
`tests/reference/gen_perft_suite.py`, the fixed sections are recorded by hand).

Startpos `d1` is unaffected by §3.1c — no zone is live there — so only `d2` moved. The two
corrections this number has taken, both settled on the chess.com analysis board:

| d1 | d2 | rule state |
|----|----|------------|
| 1878 | 3287752 | plus-shaped caster block — the bug in public commit `5264a3f7` |
| 1814 | 3061102 | full-3x3 caster block restored (2026-07-14, RainRat's PR #5) |
| 1814 | 3041742 | plus §3.1c, the enemy-zone centre is not a gate (2026-08-21, sscg13's report) |

**Historical note**: `1878` came from the public branch relaxing the caster block to orthogonal
adjacency; RainRat's PR #5 challenged it and a direct chess.com experiment confirmed the full 3x3
(diagonal king blocked, origin-based, out-of-zone free). The frozen FSF baseline binary carried
the plus-shape; the reference fork has since taken both fixes, so the suite is measured against
`FSF PR#96 f3c0d204 + the §3.1c fix` and the cross-check is clean on all 168 rows.
