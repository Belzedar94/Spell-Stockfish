#!/usr/bin/env python3
"""PyTorch mirror of the quantized Spell-NNUE A architecture.

Same chassis and quantization grids as :mod:`model` (Spell-NNUE v2); the FT
reads a single flat block of 1,182 inputs. With no king buckets there is
nothing to factorize on the freeze-gate rows either, so the model has exactly
one embedding table per output head.
"""
...
import features_a
import spla
from model import (FC0_WEIGHT_SCALE, FC1_WEIGHT_SCALE, FC2_WEIGHT_SCALE, FT_SCALE, HIDDEN_ONE,
                   PSQT_SCALE, _fake_floor, _quantize, _trunc_div)
...
class SpellNNUEA(nn.Module):
    """1,182 -> 1024 pairwise FT and sixteen 32/64/32/128 stacks."""

    def __init__(self, seed: int = 1) -> None:
        super().__init__()
        torch.manual_seed(seed)

        sigma = (1.0 / features_a.INPUT_DIMS) ** 0.5
        self.ft_bias = nn.Parameter(torch.empty(spla.L1).uniform_(-sigma, sigma))
        self.ft_weight = nn.Parameter(
            torch.empty(spla.SPELL_DIMS, spla.L1).uniform_(-sigma, sigma))
        self.psqt_weight = nn.Parameter(torch.zeros(spla.SPELL_DIMS, spla.PSQT_BUCKETS))

        self.fc0_weight = nn.Parameter(torch.empty(spla.STACKS, spla.FC0_OUT, spla.L1))
        self.fc0_bias = nn.Parameter(torch.empty(spla.STACKS, spla.FC0_OUT))
        self.fc1_weight = nn.Parameter(torch.empty(spla.STACKS, spla.FC1_OUT, spla.FC1_IN))
        self.fc1_bias = nn.Parameter(torch.empty(spla.STACKS, spla.FC1_OUT))
        self.fc2_weight = nn.Parameter(torch.empty(spla.STACKS, 1, spla.FC2_IN))
        self.fc2_bias = nn.Parameter(torch.zeros(spla.STACKS, 1))
        self._init_stacks()
    ...
    def _init_material_psqt(self) -> None:
        values = (0.0, 126.0, 781.0, 825.0, 1276.0, 2538.0)
        for piece_type in range(1, 6):
            value = values[piece_type] / 600.0
            own = (piece_type - 1) * 128
    ...
    def forward(self, batch: SparseBatch, quantized_activations: bool = True) -> torch.Tensor:
        accumulation = self._bags(self.ft_weight, batch.spell_indices, batch.offsets)
        accumulation = accumulation + self.ft_bias
        ...
        psqt = self._bags(self.psqt_weight, batch.spell_indices, batch.offsets).reshape(
            -1, 2, spla.PSQT_BUCKETS)
...
@torch.no_grad()
def quantized_params(model: SpellNNUEA) -> dict:
    model.clip_weights_()
    params = {
        "ft_bias": _quantize(model.ft_bias, FT_SCALE, np.int16),
        "ft_weight": _quantize(model.ft_weight, FT_SCALE, np.int16),
        "psqt_weight": _quantize(model.psqt_weight, PSQT_SCALE, np.int32),
        "stacks": [],
    }
    for bucket in range(spla.STACKS):
...
def write_model(model: SpellNNUEA, path: str, description: str) -> None:
    spla.write_spla(path, quantized_params(model), description)


def quantized_forward(params: dict, item: features_a.FeatureIndices,
                      stm: int) -> tuple[int, int, int]:
    """Integer reference matching ``SpellA::raw_evaluate`` exactly."""
    ...
    for psq in (item.psq_white, item.psq_black):
        ...
        ps = np.zeros(spla.PSQT_BUCKETS, dtype=np.int64)
        if psq:
