#!/usr/bin/env python3
"""Training driver for the Spell-NNUE A architecture.

Same loss, lambda schedule and optimizers as :mod:`train_overfit` (the v2
driver); the model has one embedding table per head and no factorization, so
the parameter groups are simpler. Run it on a small slice first to reproduce
the overfit gate, then on the full corpus.
"""
...
import run7
import spla
from train_overfit import MATE_TARGET_CP, training_score
        ...
        with mmap.mmap(file.fileno(), 0, access=mmap.ACCESS_READ) as mapped:
            for start in range(0, count, batch_size):
                ...
                for index in range(start, stop):
                    ...
                    # A king capture ends the game; the record that follows it
                    # has no king to place in the own-king plane.
                    if run7.W_KING not in record.board or run7.B_KING not in record.board:
                        continue
                    extracted.append(features_a.extract(record))
...
def load_init_net(net, path: str) -> None:
    """Initialize the float model from a quantized SPLA file (RL continuation).

    Dequantizes with the exact inverse of ``model_a.quantized_params``. The
    round trip costs a small precision loss; prefer ``--init-checkpoint``
    floats when they are available.
    """
    params, desc = spla.read_spla(path)
    with torch.no_grad():
        net.ft_bias.copy_(torch.from_numpy(
            params["ft_bias"].astype("float32") / model_a.FT_SCALE))
        net.ft_weight.copy_(torch.from_numpy(
            params["ft_weight"].astype("float32") / model_a.FT_SCALE))
        net.psqt_weight.copy_(torch.from_numpy(
            params["psqt_weight"].astype("float32") / model_a.PSQT_SCALE))
        for b, s in enumerate(params["stacks"]):
            net.fc0_bias[b].copy_(torch.from_numpy(
                s["fc0_bias"].astype("float32")
                / (model_a.FC0_WEIGHT_SCALE * model_a.HIDDEN_ONE)))
            net.fc0_weight[b].copy_(torch.from_numpy(
                s["fc0_weight"].astype("float32") / model_a.FC0_WEIGHT_SCALE))
            net.fc1_bias[b].copy_(torch.from_numpy(
                s["fc1_bias"].astype("float32")
                / (model_a.FC1_WEIGHT_SCALE * model_a.HIDDEN_ONE)))
            net.fc1_weight[b].copy_(torch.from_numpy(
                s["fc1_weight"].astype("float32") / model_a.FC1_WEIGHT_SCALE))
            net.fc2_bias[b].copy_(torch.from_numpy(
                s["fc2_bias"].astype("float32")
                / (model_a.FC2_WEIGHT_SCALE * model_a.HIDDEN_ONE)))
            net.fc2_weight[b].copy_(torch.from_numpy(
                s["fc2_weight"].astype("float32") / model_a.FC2_WEIGHT_SCALE))
...
def train(args) -> dict:
    ...
    net = model_a.SpellNNUEA(seed=args.seed)
    if args.init_net:
    ...
    sparse_parameters = [net.ft_weight, net.psqt_weight]
    ...
    summary = {
        "architecture": "SpellAv2 (flat, no king buckets, no threats)",
    ...
    model_a.write_model(net, args.out,
                        f"SpellAv2 {args.records} records x {args.epochs} epochs "
                        f"lambda {args.start_lambda}->{args.end_lambda}")
    if args.checkpoint:
        torch.save({"model": net.state_dict(), "summary": summary}, args.checkpoint)

    print(f"TRAIN {'CONVERGED' if converged else 'FLAT'}: "
...
def main() -> None:
    ...
    parser.add_argument("--out", required=True, help="trained SPLA .nnue")
    parser.add_argument("--curve", help="optional JSON loss curve")
    parser.add_argument("--checkpoint", help="optional PyTorch checkpoint")
    parser.add_argument("--init-net", help="SPLA .nnue to initialize from (dequantized)")
