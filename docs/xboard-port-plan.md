...
| Relojes | `level MPS BASE INC` (BASE puede ser `m:ss`), `st N` (s/jugada), `sd N` (depth), `time N` / `otim N` (**centisegundos**) | `time`/`otim` llegan antes del usermove; asignación por playColor con fallback a side_to_move en modo force (playColor==COLOR_NB), y solo actualizan el reloj si `level` ya lo inicializó (FSF xboard.cpp:289-302) |
...
- Smoke test manual en WinBoard con variant spell-chess (capturar el `setup` exacto del oráculo; OJO: spell-chess NO activa pieceDrops — pocketsize será 0 y los contadores de hechizos viajan en el FEN `[holdings]`, no como pockets CECP) y en cutechess-cli -protocol xboard motor-vs-oráculo, >100 partidas por el protocolo de 3 TCs.
