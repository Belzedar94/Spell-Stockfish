...
moves_loop:  // When in check, search starts here
    ...
    while ((move = mp.next_move()) != Move::none())
    {
        ...
        // Spell state of this capture, read on the board as it stands now — the
        // same view MovePicker scored it from and the one update_all_stats will
        // train. It has to be taken before the move: after it, the defenders of
        // the destination square are no longer the ones that were asked to
        // answer the capture.
        const int capFreeze = capture ? capture_freeze_bucket(pos, move.to_sq(), ~us) : 0;
    ...
    else if (priorCapture && prevSq != SQ_NONE)
    {
        ...
        // The capture being rewarded was made by the opponent, so we are the
        // side whose frozen defenders it walked past. The board has already
        // advanced past that move: what is measured here is the state the
        // recapture would actually face.
        captureHistory[pos.piece_on(prevSq)][prevSq][type_of(capturedPiece)]
                      [capture_freeze_bucket(pos, prevSq, us)]
          << 901;
    }
...
void update_all_stats(const Position& pos,
    ...
    // Every move updated below is one of ours, so the side that would answer a
    // capture is the opponent. All moves are already undone: the freeze buckets
    // are read on the same board MovePicker scored these captures from.
    ...
    else
    {
        ...
        captureHistory[movedPiece][bestMove.to_sq()][capturedPiece]
                      [capture_freeze_bucket(pos, bestMove.to_sq(), them)]
          << bonus * 1366 / 1024;
    }
    ...
    for (Move move : capturesSearched)
    {
        ...
        captureHistory[movedPiece][move.to_sq()][capturedPiece]
                      [capture_freeze_bucket(pos, move.to_sq(), them)]
          << -malus * 1518 / 1024;
    }
}
