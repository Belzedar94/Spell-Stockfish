...
Value Search::Worker::evaluate(const Position& pos) {
    ...
    // The cooldown-window term rides on top of whichever network answered: this
    // is the single funnel every static evaluation passes through, so it is
    // added exactly once, and the cooldowns are part of the Zobrist key, so
    // transposition-table static evals stay consistent with it.
    return std::clamp(v + Eval::spell_cooldown_penalty(pos), VALUE_TB_LOSS_IN_MAX_PLY + 1,
