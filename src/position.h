...
// Spell chess: a piece standing in an enemy freeze zone cannot move, so it
// threatens nothing while the zone lasts. attackers_to() already drops those
// men from its attacker set; do the same here instead of reading attacks_bb()
// straight off the piece bitboards.
template<PieceType Pt>
inline Bitboard Position::attacks_by(Color c) const {

    const Bitboard active = ~frozen_squares(c);

    if constexpr (Pt == PAWN)
        return c == WHITE ? pawn_attacks_bb<WHITE>(pieces(WHITE, PAWN) & active)
                          : pawn_attacks_bb<BLACK>(pieces(BLACK, PAWN) & active);
    else
    {
        Bitboard threats   = 0;
        Bitboard attackers = pieces(c, Pt) & active;
        while (attackers)
