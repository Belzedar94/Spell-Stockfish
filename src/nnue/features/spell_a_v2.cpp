...
IndexType SpellAv2::make_spell_index(Color perspective, const DirtySpellEvent& ev) {
    switch (ev.block())
    {
    case DirtySpellEvent::FREEZE_GATE :
        return make_freeze_index(perspective, ev.color(), Square(ev.index()));
...
void SpellAv2::append_active_spell(Color perspective, const Position& pos, IndexList& active) {
    for (Color c : {WHITE, BLACK})
    {
        // Live zone gates
        const Square gF = pos.spell_gate(c, SPELL_FREEZE);
        if (gF != SQ_NONE)
            active.push_back(make_freeze_index(perspective, c, gF));
...
void SpellAv2::append_changed_indices(Color             perspective,
                                    const DirtyPiece& dp,
                                    const DirtySpell& ds,
                                    IndexList&        removed,
                                    IndexList&        added) {
    // Piece delta (absent for null-move states)
    if (!ds.isNull)
    {
        removed.push_back(make_index(perspective, dp.from, dp.pc));
        if (dp.to != SQ_NONE)
            added.push_back(make_index(perspective, dp.to, dp.pc));

        if (dp.remove_sq != SQ_NONE)
            removed.push_back(make_index(perspective, dp.remove_sq, dp.remove_pc));

        if (dp.add_sq != SQ_NONE)
            added.push_back(make_index(perspective, dp.add_sq, dp.add_pc));
    }

    // Board spell events remain individual rows. GLOBAL thermometer events
    // are folded into one exact old->new row by FeatureTransformerA.
    for (const auto& ev : ds.list)
        if (ev.block() != DirtySpellEvent::GLOBAL)
            (ev.add() ? added : removed).push_back(make_spell_index(perspective, ev));
}
